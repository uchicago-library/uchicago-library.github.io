...
* Introduction
  ...
  The [[*Guide][Guide]] section is intended as a quick walkthrough to get you going bringing
  a new digital collection on board.  The [[*Reference][Reference]] section provides a more
...
* Guide
  ...
  + load the relevant collection data onto our IIIF and Loris servers, where
    applicable
...
** Mark Logic Preparation
   ...
   objects, along with all the metadata we have for each object.  Metadata for
   each object would include information such as: who created it, when was it
   created, who is the publisher, what geographic regions does it cover (if
   applicable), etc.
...
** IIIF Preparation
   ...
   + provide a URL the [[https://universalviewer.io/][Universal Viewer]] app can use to live-display a collection
   ...
   collection objects under categories, and provide a link to the IIIF manifest
   for each object.
...
*** ARK Identifiers
    ...
    disadvantage to using a DOI as the primary identifier for our objects is
    that DOI resolution is proprietary, which means that every time you want to
    look an object up by DOI, you need to use the CrossRef website.  We would
    prefer to be able to look objects in our collection up without being
    beholden to a third-party lookup service and whatever baggage comes along
    with it, in terms of performance, service outage, and compatibility-breaking
    upgrades.
    ...
    know what you're thinking, but please keep your early 80s arcade game puns
    to yourself.)
...
* Reference
  ...
  [ INSERT GENERAL DESCRIPTION OF REFERENCE SECTION HERE ]
