...
regular expression string that says =^temporary_route/$=.  What that means is:
whatever the root URL for this page is, add =temporary_route/= to the end of
that and the app will direct to the template you provided the path to.
