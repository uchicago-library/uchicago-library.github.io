...
create a route, but one simple way is to add the following function definition
to the =CollectionPage= template in =lib_collections/models.py=.  In the context
of our example, it would look like this:
...
regular expression string that says =^temporary_route/$=.  Translated into
English, that means: whatever the root URL for any page of this type ends up
being, add =temporary_route/= to the end of that and the app will direct to the
template you provided the path to.

** Finishing Up

Personally, I think it would be great tot be handed the project exactly in the
